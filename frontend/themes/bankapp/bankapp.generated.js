...
window.Vaadin.Flow['_vaadintheme_bankapp_globalCss'] = window.Vaadin.Flow['_vaadintheme_bankapp_globalCss'] || [];
export const applyTheme = (target) => {
  
  const injectGlobal = (window.Vaadin.Flow['_vaadintheme_bankapp_globalCss'].length === 0) || (!window.Vaadin.Flow['_vaadintheme_bankapp_globalCss'].includes(target) && target !== document);
  if (injectGlobal) {
    injectGlobalCss(stylesCss.toString(), target);
    
    window.Vaadin.Flow['_vaadintheme_bankapp_globalCss'].push(target);
  }
  if (!document['_vaadintheme_bankapp_componentCss']) {
    
    document['_vaadintheme_bankapp_componentCss'] = true;
  }
